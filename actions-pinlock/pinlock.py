...
USES_PATTERN = re.compile(
    r"^(\s*uses:\s+)([a-zA-Z0-9_-]+/[a-zA-Z0-9_-]+)@([a-zA-Z0-9.]+)(.*)$",
    re.MULTILINE
)
...
class Pinlock:
    ...
    def update_file(
        self, filepath: Path, updates: List[Tuple[str, str, str]]
    ) -> str:
        """
    ...
    def create_branch_name(self) -> str:
        ...
        return f"pinlock/update-actions-{timestamp}"

    def create_pr_body(
        self, mismatches: Dict[Path, List[Tuple[str, str, str]]]
    ) -> str:
        ...
        for action_name in sorted(all_updates.keys()):
            ...
            old_short = (
                old_version[:7] if len(old_version) > 10 else old_version
            )
            new_short = (
                new_version[:7] if len(new_version) > 10 else new_version
            )
            lines.append(
                f"- `{action_name}`: `{old_short}...` → `{new_short}...`"
            )
    ...
    def run(self):
        ...
        if not self.no_pr:
            ...
            if result.returncode == 0:
                ...
                print(
                    "⚠️  Failed to create PR (you may need to install gh CLI)"
                )
...
def main():
    ...
    parser.add_argument(
        "--no-pr",
        action="store_true",
        help="Push branch but don't create PR",
    )
